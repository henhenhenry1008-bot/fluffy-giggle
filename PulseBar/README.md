...
The project has a compilable menu bar dashboard and a centralized one-second refresh loop. CPU usage is measured from real system-wide Mach tick counters, and memory usage comes from Mach VM statistics plus the physical memory size reported by `sysctl`. Network, disk, and battery still come from clearly named placeholder services. Charts, temperature sensors, and privileged operations are not implemented yet.
