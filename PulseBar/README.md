...
The project has a compilable menu bar dashboard, one centralized and cancellable refresh loop with supported intervals of 0.5, 1, 2, and 5 seconds, and bounded real-time charts for CPU, memory, and network throughput. CPU usage is measured from real system-wide Mach tick counters, memory usage comes from Mach VM statistics plus the physical memory size reported by `sysctl`, network throughput is calculated from native 64-bit BSD interface counters, disk capacity comes from Foundation volume resource values, and battery state comes from IOKit power source APIs. Batteryless Macs report the metric as unavailable. Temperature sensors and privileged operations are not implemented yet.
